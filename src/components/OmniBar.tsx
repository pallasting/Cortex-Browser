    ...
    <div className="flex items-center gap-2 p-2 bg-cortex-panel border-b border-cortex-border sticky top-0 z-50">
      ...
      <div className="flex bg-black/40 rounded-lg p-0.5 border border-cortex-border">
        ...
        >
         ...
         <button 
          onClick={() => onViewModeChange(ViewMode.NEURO)}
          className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-md transition-all ${viewMode === ViewMode.NEURO ? 'bg-purple-500/20 text-purple-500 border border-purple-500/30 shadow-sm' : 'text-slate-400 hover:text-purple-500'}`}
          title="NeuroRust Physics Engine"
        >
          <ActivityIcon /> Neuro
        </button>
      </div>
